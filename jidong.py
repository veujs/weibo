...
def crow_first(n):
    ...
    #     2 * n - 1)
    url = 'https://search.jd.com/Search?keyword=ipad&enc=utf-8&wq=ipad&pvid=c928dfd805a3462f8d23610c0cd7e992' + str(
        2 * n - 1)
    ...
    print('3333333333333333333333333333333333 %s ' % datas)
    for i in datas:
        print(222222222222)
    # 将抓取的结果保存到本地CSV文件中
    with open('JD_Phone.csv', 'w', newline='', encoding='utf-8')as f:
        write = csv.writer(f)

        print(len(datas))
        for data in datas:
            write.writerow("444444444444444444444")
            p_price = data.xpath('div/div[@class="p-price"]/strong//text()')
            # print(etree.tostring(p_price))
